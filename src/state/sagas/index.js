...
/** Минусы:
    - Не избавляемся от писанины в компанентах
    - Сложность кода в сравнении с thunk и async/await
    - Время на чтение чужого кода в случае не знания API
    - Время на изучение ее работы новичку
    - Скорость написания (из-за появления нового магического слоя - приходится танцевать с бубном)
    - Generator - старая дичь, которую я так и не припомню где используют кроме как в Саге
 */

/** Плюсы:
    - Легко тестируема - но у нас не будет тестов - (сомнительный плюс)
    - Гибкий инструмент с разнообразным API для различных ситуаций
    - Подходит для больших приложений где много разработчиков и тестов
 */

// Интеграция с мессенджарами в одном месте- хотя бы уведомления
